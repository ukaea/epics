...
long epicsShareAPI dbgrep(const char *pmask)
{
    ...
    while (!status) {
        ...
        while (!status) {
            ...
            if (epicsStrGlobMatch(pname, pmask)) puts(pname);
            status = dbNextRecord(pdbentry);
        }
        status = dbNextRecordType(pdbentry);
    }
...
long epicsShareAPI dbgf(const char *pname)
{
    ...
    if (addr.dbr_field_type == DBR_ENUM) {
        status = dbGetField(&addr, DBR_STRING, pbuffer,
            &options, &no_elements, NULL);
        printBuffer(status, DBR_STRING, pbuffer, 0L, 0L,
            no_elements, &msg_Buff, tab_size);
    } else {
...
long epicsShareAPI dbtpf(const char *pname, const char *pvalue)
{
    ...
    for (put_type = DBR_STRING; put_type <= DBF_ENUM; put_type++) {
        ...
        case DBR_CHAR:
            if ((valid = validLong)) {
                val_i8 = (epicsInt8)val_long;
                status = dbPutField(&addr, put_type, &val_i8, 1L);
            }
            break;
        case DBR_UCHAR:
            if ((valid = validULong)) {
                val_u8 = (epicsUInt8)val_ulong;
