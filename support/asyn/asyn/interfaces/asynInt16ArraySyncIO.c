...
***********************************************************************/
/*
 * This package provide a simple, synchronous interface to asynInt16Array
 * Author:  Mark Rivers
 * Created: 13APR2008
 */
...
/*asynInt16ArraySyncIO methods*/
static asynStatus connect(const char *port, int addr,
                          asynUser **ppasynUser, const char *drvInfo);
static asynStatus disconnect(asynUser *pasynUser);
static asynStatus writeOp(asynUser *pasynUser,epicsInt16 *pvalue,size_t nelem,double timeout);
...
static asynInt16ArraySyncIO interface = {
    connect,
    disconnect,
    writeOp,
    readOp,
   ...
   asynUser **ppasynUser, const char *drvInfo)
{
    ...
    pioPvt = (ioPvt *)callocMustSucceed(1, sizeof(ioPvt),"asynInt16ArraySyncIO");
    ...
    *ppasynUser = pasynUser;
    ...
    if (!pasynInterface) {
       epicsSnprintf(pasynUser->errorMessage,pasynUser->errorMessageSize,
           "interface %s is not supported by port",asynInt16ArrayType);
       return asynError;
    }
    pioPvt->pasynInt16Array = (asynInt16Array *)pasynInterface->pinterface;
    pioPvt->int16ArrayPvt = pasynInterface->drvPvt;
    if(drvInfo) {
...
static asynStatus writeOp(asynUser *pasynUser,epicsInt16 *pvalue,size_t nelem,double timeout)
{
    ...
    status = pPvt->pasynInt16Array->write(pPvt->int16ArrayPvt, pasynUser,pvalue,nelem);
    if (status==asynSuccess) {
    ...
    epicsInt16 *pvalue,size_t nelem,double timeout,const char *drvInfo)
{
    ...
    if(status!=asynSuccess) {
       asynPrint(pasynUser, ASYN_TRACE_ERROR,
           "asynInt16ArraySyncIO connect failed %s\n",
    ...
    if(status!=asynSuccess) {
       asynPrint(pasynUser, ASYN_TRACE_ERROR,
            "asynInt16ArraySyncIO writeOp failed %s\n",pasynUser->errorMessage);
    }
    disconnect(pasynUser);
    return status;
}
                   ...
                   epicsInt16 *pvalue,size_t nelem,size_t *nIn,double timeout,const char *drvInfo)
{
    ...
    if(status!=asynSuccess) {
       asynPrint(pasynUser, ASYN_TRACE_ERROR,
            "asynInt16ArraySyncIO readOp failed %s\n",pasynUser->errorMessage);
    }
    disconnect(pasynUser);
    return status;
}
