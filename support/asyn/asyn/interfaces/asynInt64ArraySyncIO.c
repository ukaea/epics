...
***********************************************************************/
/*
 * This package provide a simple, synchronous interface to asynInt64Array
 * Author:  Mark Rivers
 * Created: 13APR2008
 */
...
#include "asynInt64ArraySyncIO.h"

typedef struct ioPvt{
   asynCommon   *pasynCommon;
   void         *pcommonPvt;
   asynInt64Array  *pasynInt64Array;
   void         *int64ArrayPvt;
   asynDrvUser  *pasynDrvUser;
   void         *drvUserPvt;
}ioPvt;

/*asynInt64ArraySyncIO methods*/
static asynStatus connect(const char *port, int addr,
                          asynUser **ppasynUser, const char *drvInfo);
static asynStatus disconnect(asynUser *pasynUser);
static asynStatus writeOp(asynUser *pasynUser,epicsInt64 *pvalue,size_t nelem,double timeout);
static asynStatus readOp(asynUser *pasynUser,epicsInt64 *pvalue,size_t nelem,size_t *nIn,double timeout);
...
static asynInt64ArraySyncIO interface = {
    connect,
    disconnect,
    writeOp,
    readOp,
   ...
   asynUser **ppasynUser, const char *drvInfo)
{
    ...
    pioPvt = (ioPvt *)callocMustSucceed(1, sizeof(ioPvt),"asynInt64ArraySyncIO");
    ...
    *ppasynUser = pasynUser;
    ...
    pasynInterface = pasynManager->findInterface(pasynUser, asynInt64ArrayType, 1);
    if (!pasynInterface) {
       epicsSnprintf(pasynUser->errorMessage,pasynUser->errorMessageSize,
           "interface %s is not supported by port",asynInt64ArrayType);
       return asynError;
    }
    pioPvt->pasynInt64Array = (asynInt64Array *)pasynInterface->pinterface;
    pioPvt->int64ArrayPvt = pasynInterface->drvPvt;
    if(drvInfo) {
...
static asynStatus writeOp(asynUser *pasynUser,epicsInt64 *pvalue,size_t nelem,double timeout)
{
    ...
    status = pPvt->pasynInt64Array->write(pPvt->int64ArrayPvt, pasynUser,pvalue,nelem);
    if (status==asynSuccess) {
    ...
    epicsInt64 *pvalue,size_t nelem,double timeout,const char *drvInfo)
{
    ...
    if(status!=asynSuccess) {
       asynPrint(pasynUser, ASYN_TRACE_ERROR,
           "asynInt64ArraySyncIO connect failed %s\n",
    ...
    if(status!=asynSuccess) {
       asynPrint(pasynUser, ASYN_TRACE_ERROR,
            "asynInt64ArraySyncIO writeOp failed %s\n",pasynUser->errorMessage);
    }
    disconnect(pasynUser);
    return status;
}
                   ...
                   epicsInt64 *pvalue,size_t nelem,size_t *nIn,double timeout,const char *drvInfo)
{
    ...
    if(status!=asynSuccess) {
       asynPrint(pasynUser, ASYN_TRACE_ERROR,
            "asynInt64ArraySyncIO readOp failed %s\n",pasynUser->errorMessage);
    }
    disconnect(pasynUser);
    return status;
}
